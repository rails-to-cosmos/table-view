...
instance SP.ManagedPtrNewtype MediaKeySystemPermissionRequest where
    toManagedPtr (MediaKeySystemPermissionRequest p) = p
...
instance B.Types.TypedObject MediaKeySystemPermissionRequest where
    glibType = c_webkit_media_key_system_permission_request_get_type

instance B.Types.GObject MediaKeySystemPermissionRequest

-- | Type class for types which can be safely cast to t'MediaKeySystemPermissionRequest', for instance with `toMediaKeySystemPermissionRequest`.
...
-- | Cast to t'MediaKeySystemPermissionRequest', for types for which this is known to be safe. For general casts, use 'Data.GI.Base.ManagedPtr.castTo'.
toMediaKeySystemPermissionRequest :: (MIO.MonadIO m, IsMediaKeySystemPermissionRequest o) => o -> m MediaKeySystemPermissionRequest
toMediaKeySystemPermissionRequest = MIO.liftIO . B.ManagedPtr.unsafeCastTo MediaKeySystemPermissionRequest

-- | Convert t'MediaKeySystemPermissionRequest' to and from t'Data.GI.Base.GValue.GValue'. See 'Data.GI.Base.GValue.toGValue' and 'Data.GI.Base.GValue.fromGValue'.
instance B.GValue.IsGValue (Maybe MediaKeySystemPermissionRequest) where
    gvalueGType_ = c_webkit_media_key_system_permission_request_get_type
    gvalueSet_ gv P.Nothing = B.GValue.set_object gv (FP.nullPtr :: FP.Ptr MediaKeySystemPermissionRequest)
    gvalueSet_ gv (P.Just obj) = B.ManagedPtr.withManagedPtr obj (B.GValue.set_object gv)
    gvalueGet_ gv = do
        ptr <- B.GValue.get_object gv :: IO (FP.Ptr MediaKeySystemPermissionRequest)
        if ptr /= FP.nullPtr
        then P.Just <$> B.ManagedPtr.newObject MediaKeySystemPermissionRequest ptr
